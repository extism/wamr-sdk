...
static uint64_t native_alloc(wasm_exec_env_t env, uint64_t size) {
  ...
  wasm_runtime_set_module_inst(env, kernel->instance);
...
ExtismStatus extism_plugin_init(ExtismPlugin *plugin,
  ...
  // assert(wasm_runtime_register_module("extism:host/env", kernel_mod,
  // errmsg,
  //                                     1024));
...
static uint64_t kernel_alloc(struct ExtismKernel *kernel, const void *s,
                             size_t size) {
  wasm_val_t params[] = {{.kind = WASM_I64, .of = {.i64 = size}}};
  wasm_val_t results[] = {{.kind = WASM_I64, .of = {.i64 = 0}}};
  assert(wasm_runtime_call_wasm_a(kernel->exec, kernel->alloc, 1, results, 1,
                                  params));
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    if (!wasm_runtime_call_wasm_a(kernel->exec, kernel->store_u8, 0, NULL, 2,
                                  params)) {
...
static void kernel_set_input(struct ExtismKernel *kernel, uint64_t offs,
                             size_t length) {
  wasm_val_t params[] = {{.kind = WASM_I64, .of = {.i64 = offs}},
                         {.kind = WASM_I64, .of = {.i64 = length}}};
  wasm_runtime_call_wasm_a(kernel->exec, kernel->input_set, 0, NULL, 2, params);
}

static uint64_t kernel_output_offset(struct ExtismKernel *kernel) {
  wasm_val_t results[] = {{.kind = WASM_I64, .of = {.i64 = 0}}};
  wasm_runtime_call_wasm_a(kernel->exec, kernel->output_offset, 1, results, 0,
                           NULL);
  return results[0].of.i64;
}

static uint64_t kernel_output_length(struct ExtismKernel *kernel) {
  wasm_val_t results[] = {{.kind = WASM_I64, .of = {.i64 = 0}}};
  wasm_runtime_call_wasm_a(kernel->exec, kernel->output_length, 1, results, 0,
                           NULL);
  return results[0].of.i64;
}

static void kernel_reset(struct ExtismKernel *kernel) {
  wasm_runtime_call_wasm_a(kernel->exec, kernel->reset, 0, NULL, 0, NULL);
}
...
ExtismStatus extism_plugin_call(ExtismPlugin *plugin, const char *func_name,
  ...
  // kernel_reset(&plugin->kernel);
  uint64_t input_offs = kernel_alloc(&plugin->kernel, input, input_length);
  kernel_set_input(&plugin->kernel, input_offs, input_length);
...
uint8_t *extism_plugin_output(ExtismPlugin *plugin, size_t *length) {
  if (length) {
    *length = kernel_output_length(&plugin->kernel);
  }
  return wasm_runtime_addr_app_to_native(plugin->instance,
                                         kernel_output_offset(&plugin->kernel));
}
